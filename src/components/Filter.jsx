...
const Filter = () => {
  ...
  return (
    <>
      <div className="cmn-parent-bg mt-5 p-2  ">
        <Carousel
          opts={{
            align: "start",
          }}
        ...
        >
          <CarouselContent className="flex items-center">
            {categorydata.map((value, index) => (
              <CarouselItem
                key={index}
                className="basis-2/3 sm:basis-2/3 md:basis-1/3 lg:basis-1/4 xl:basis-1/5"
              >
                <button
                  onClick={() => handleUpadteCategory(value)}
                  className={` ${
                    value === select ? "bg-[#d78330]" : ""
                  } w-full  cmn-child-bg  text-sm sm:text-base  main-text font-bold shadow-lg flex items-center justify-center rounded-full p-2`}
                >
                  {value}
                </button>
              </CarouselItem>
